...
namespace Soulcalibur.Tests
{
    public abstract class SoulcaliburTestBase : AbpIntegratedTestBase<SoulcaliburTestModule>
    {
        ...
        protected void UsingDbContext(Action<SoulcaliburDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SoulcaliburDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SoulcaliburDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SoulcaliburDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SoulcaliburDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SoulcaliburDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SoulcaliburDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SoulcaliburDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SoulcaliburDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SoulcaliburDbContext, Task<T>> func)
        {
            T result;
